(function () {
  ...
  function applyLang(lang) {
    ...
    try { localStorage.setItem("astreet-lang", lang); } catch (e) {}
  }
  document.querySelectorAll(".lang button").forEach(function (btn) {
    btn.addEventListener("click", function () { applyLang(btn.getAttribute("data-set")); });
  });
  var saved = null;
  try { saved = localStorage.getItem("astreet-lang"); } catch (e) {}
  ...
  if (form) {
    form.addEventListener("submit", function (e) {
      ...
      var body = '"A" Street Auto Service job\nName: ' + name + (vehicle ? "\nVehicle: " + vehicle : "") + "\nNeed: " + need;
